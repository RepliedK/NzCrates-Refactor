...
class PegasusBoxEntity extends Living {
    ...
    // Crear y añadir la partícula en la posición calculada de la espiral
    ...
    public function openFormVote(Player $player): void {
        $vote = Main::getKeyType($player, "pegasus");
        $form = new SimpleForm(function(Player $player, $args = null) use ($vote) {
            ...
            switch ($args) {
                ...
                case 1:
                    ...
                    if ($vote !== 0) {
                        Main::removeKeyBox($player, "pegasus", 1);
                        Main::getInstance()->getCrateManager()->getRandomItemFromCrate("pegasus", $player->getName(), $this, "animation.dark_magma.opened", "controller.animation.dark_magma.opened");
        ...
        $config = Main::getInstance()->getConfig();
        $titleText = $config->get("pegasusformtitle");
        $formattedTitleText = str_replace("\n", "\n", $titleText);
        $contentText = $config->get("pegasusformcontent");
